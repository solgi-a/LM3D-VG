"""
Parser ablation, variant A -- GPT-4o-mini (the paper's main configuration).

Reference arm of the parser ablation. Reads ``data_parsing/final_parsing_tokenized``,
which ships with the repo.

    python experiments/ablation/runners/run_parser_gpt.py
...
TAG = "ABL-PARSER-GPT"                        # output folder suffix under outputs/
PARSING_FOLDER = "final_parsing_tokenized"    # variant A: GPT-4o-mini
...
def main():
    ...
    if not RUN:
        ...
        return 0
    cmd = build_command()
    print("[ablation] parser variant A (GPT-4o-mini)")
