...
def check_parse_cache():
    ...
    if missing:
        print(f"ERROR: no clipped LLaMA parse cache for split(s) {missing} in {folder}")
        print("The raw LLaMA cache exceeds the 7/17/75 token caps and would crash "
              "pack_padded_sequence mid-training. Generate the clipped copy first:")
        print(f"    python experiments/ablation/parsers/clip_parse_cache.py \\")
        print(f"        --input  data/scannet/{SOURCE_FOLDER} \\")
        print(f"        --output data_parsing/{PARSING_FOLDER}")
        return False
    return True
...
def main():
    ...
    if not check_parse_cache():
        return 1
    cmd = build_command()
    print("[ablation] parser variant C (LLaMA, token caps enforced)")
