"""
Parser ablation, variant D -- no parser.

    RUNS ON: GPU. One training run (~2-3 h under the cached-scene protocol).

The architecture is untouched -- A2F and TAF still run -- and only the parse content is
removed, so the difference against variants A/B/C/E is attributable to the parser alone.

Build the empty parse cache first (~5 s on CPU), then train:

    python experiments/ablation/parsers/make_noparse_cache.py --splits train val
    python experiments/ablation/runners/run_parser_none.py

All three fields carry the single token ``unk`` rather than zeros; see that script's
docstring for why.
"""
...
TAG = "ABL-PARSER-NONE"                       # output folder suffix under outputs/
PARSING_FOLDER = "noparse_tokenized"          # variant D: no parser, fields = ["unk"]
...
def check_parse_cache():
    ...
    if missing:
        print(f"ERROR: no no-parse cache for split(s) {missing} in {folder}")
        print("Generate it first:")
        print("    python experiments/ablation/parsers/make_noparse_cache.py --splits train val")
        return False
    return True
...
def main():
    ...
    if not check_parse_cache():
        return 1
    cmd = build_command()
    print("[ablation] parser variant D (no parser; A2F and TAF still active)")
