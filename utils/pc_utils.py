...
def point_cloud_to_volume(points, vsize, radius=1.0):
    """ points: Nx3 in [-radius,radius] -> vsize^3 occupancy grid """
...
def volume_to_point_cloud(vol):
    """ vol: vsize^3 occupancy grid (0/1) -> Nx3 points """
    vsize = vol.shape[0]
...
def point_cloud_to_volume_v2_batch(point_clouds, vsize=12, radius=1.0, num_sample=128):
    """ point_clouds: BxNx3 -> BxVxVxVxnum_samplex3 """
    vol_list = []
    for b in range(point_clouds.shape[0]):
...
def point_cloud_to_volume_v2(points, vsize, radius=1.0, num_sample=128):
    """ points: Nx3 in [-radius,radius] -> vsize^3 x num_sample x 3; pads/replicates points per voxel to num_sample """
...
def point_cloud_to_image_batch(point_clouds, imgsize, radius=1.0, num_sample=128):
    """ point_clouds: BxNx3 -> BxIxIxnum_samplex3 """
    img_list = []
    for b in range(point_clouds.shape[0]):
...
def point_cloud_to_image(points, imgsize, radius=1.0, num_sample=128):
    """ points: Nx3 in [-radius,radius] -> imgsize x imgsize x num_sample x 3; pads/replicates points per pixel to num_sample """
...
def rotate_point_cloud(points, rotation_matrix=None):
    """ Rotate in-place around Z axis. points: (n,3) """
