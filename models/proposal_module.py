...
try:
    ...
    if ABLATION.POINTNET_IMPL == "pointnet2_python":
        ...
        from pointnet_py_adv.pn import PointnetSAModuleVotes
...
def decode_dataset_config(data_dict, dataset_config):
    if dataset_config is not None:
        ...
        # follow the input's device so the detection branch can also run on CPU
...
class ProposalModule(nn.Module):
    ...
    def forward(self, xyz, features, end_points):
        """xyz: (B,K,3), features: (B,C,K) -> scores: (B,num_proposal,2+3+NH*2+NS*4)"""
