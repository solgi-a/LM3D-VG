...
from .GF_losses import smoothl1_loss, l1_loss, SigmoidFocalClassificationLoss
...
def compute_points_obj_cls_loss_hard_topk(end_points, topk):
    ...
    gt_size = end_points['size_gts'][:, :, 0:3]
    B = gt_center.shape[0]
    K = seed_xyz.shape[1]
    K2 = gt_center.shape[1]

    point_instance_label = end_points['point_instance_label']
    object_assignment = torch.gather(point_instance_label, 1, seed_inds)
    object_assignment[object_assignment < 0] = K2 - 1
    object_assignment_one_hot = torch.zeros((B, K, K2)).to(seed_xyz.device)
    object_assignment_one_hot.scatter_(2, object_assignment.unsqueeze(-1), 1)
    ...
    padding_array = padding_array.unsqueeze(1)
    point_instance_label_mask = (point_instance_label < 0)
    point_instance_label = point_instance_label + padding_array
    point_instance_label[point_instance_label_mask] = -1
    num_gt_bboxes = torch.unique(point_instance_label).shape[0] - 1
    seed_instance_label = torch.gather(point_instance_label, 1, seed_inds)
...
def compute_objectness_loss_based_on_query_points(end_points, num_decoder_layers):
    ...
    for prefix in prefixes:
        ...
        seed_obj_gt = torch.gather(end_points['point_obj_mask'], 1, seed_inds)
        query_points_obj_gt = torch.gather(seed_obj_gt, 1, query_points_sample_inds)

        point_instance_label = end_points['point_instance_label']
        seed_instance_label = torch.gather(point_instance_label, 1, seed_inds)
        query_points_instance_label = torch.gather(seed_instance_label, 1, query_points_sample_inds)
...
def compute_box_and_sem_cls_loss(end_points, config, num_decoder_layers,
    ...
    for prefix in prefixes:
        ...
        if center_loss_type == 'smoothl1':
            ...
            center_loss = smoothl1_loss(assigned_gt_center - pred_center, delta=center_delta)
            center_loss = torch.sum(center_loss * objectness_label.unsqueeze(2)) / (torch.sum(objectness_label) + 1e-6)
        elif center_loss_type == 'l1':
            objectness_label = end_points[f'{prefix}objectness_label'].float()
            object_assignment_expand = object_assignment.unsqueeze(2).repeat(1, 1, 3)
            assigned_gt_center = torch.gather(gt_center, 1, object_assignment_expand)
            center_loss = l1_loss(assigned_gt_center - pred_center)
            center_loss = torch.sum(center_loss * objectness_label.unsqueeze(2)) / (torch.sum(objectness_label) + 1e-6)
        else:
        ...
        if heading_loss_type == 'smoothl1':
            ...
            heading_residual_normalized_loss = l1_loss(heading_residual_normalized_error)
            heading_residual_normalized_loss = torch.sum(
                heading_residual_normalized_loss * objectness_label) / (torch.sum(objectness_label) + 1e-6)
        else:
            raise NotImplementedError
        ...
        if size_cls_agnostic:
            ...
            if size_loss_type == 'smoothl1':
                ...
                size_residual_normalized_loss = l1_loss(size_residual_normalized_error)
                size_residual_normalized_loss = torch.sum(
                    size_residual_normalized_loss * objectness_label.unsqueeze(2)) / (
                                                        torch.sum(objectness_label) + 1e-6)
            else:
        ...
        sem_cls_loss = criterion_sem_cls(end_points[f'{prefix}sem_cls_scores'].transpose(2, 1), sem_cls_label)
...
def GF_get_loss(end_points, config, num_decoder_layers,
