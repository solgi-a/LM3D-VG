...
class PredictHead(nn.Module):
    ...
    def forward(self, features, base_xyz, end_points, prefix=''):
        ...
        mean_size_arr = torch.from_numpy(self.mean_size_arr.astype(np.float32)).cuda()
        mean_size_arr = mean_size_arr.unsqueeze(0).unsqueeze(0)
        ...
        size_residuals_normalized = self.size_residual_head(net).transpose(2, 1).view(
            [batch_size, num_proposal, self.num_size_cluster, 3])
