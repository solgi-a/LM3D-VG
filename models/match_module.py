...
class MatchModule(nn.Module):
    def __init__(self, args, num_proposals=256, hidden_size=128, det_channel=288, head=4, depth=3):
        super().__init__()
        ...
        self.self_attn_post = nn.ModuleList(
            MultiHeadAttention(d_model=hidden_size, d_k=hidden_size // head, d_v=hidden_size // head, h=head) for i in range(depth))


        self.cross_attn = nn.ModuleList(
            MultiHeadAttention(d_model=hidden_size, d_k=hidden_size // head, d_v=hidden_size // head, h=head) for i in range(2*depth))

        self.tgt_cross_attn = nn.ModuleList(
            MultiHeadAttention(d_model=hidden_size, d_k=hidden_size // head, d_v=hidden_size // head, h=head) for i in range(depth))

        self.adj_cross_attn = nn.ModuleList(
            MultiHeadAttention(d_model=hidden_size, d_k=hidden_size // head, d_v=hidden_size // head, h=head) for i in range(depth))

        self.ngh_cross_attn = nn.ModuleList(
            MultiHeadAttention(d_model=hidden_size, d_k=hidden_size // head, d_v=hidden_size // head, h=head) for i in range(depth))
        ...
        self.gcn3 = GCNConv(hidden_size, hidden_size, improved=True, add_self_loops=True)
        self.gcn4 = GCNConv(hidden_size, hidden_size, improved=True, add_self_loops=True)
        self.gcn5 = GCNConv(hidden_size, hidden_size, improved=True, add_self_loops=True)
        self.gcn6 = GCNConv(hidden_size, hidden_size, improved=True, add_self_loops=True)


    def dist_cal(self,objects_center):
        ...
        return dist_weights, attention_matrix_way
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.depth):
            features = self.self_attn[i](features, features, features, attention_weights=dist_weights, way=attention_matrix_way)

        graph_obj_features = features.clone()
        len_nun_max = data_dict["lang_feat_list"].shape[1]

        obj_masks = objectness_masks.bool().squeeze(2)
        ...
        num_ngh = 5
        k_neighbors = num_ngh if obj_lens.min() >= num_ngh else obj_lens.min()
        ...
        output_features = self.gcn3(output_features, edge_index)
        output_features = self.gcn4(output_features, edge_index)
        output_features = self.gcn5(output_features, edge_index)
        output_features = self.gcn6(output_features, edge_index)
        ...
        new_objects_center = objects_center.masked_fill(~obj_masks.unsqueeze(2),1e9)
        new_dist_weights, _ = self.dist_cal(new_objects_center)

        for i in range(self.depth):
            grpah_features = self.self_attn_post[i](grpah_features, grpah_features, grpah_features, attention_weights=new_dist_weights, way=attention_matrix_way)
        
        grpah_features = grpah_features.reshape(batch_size*num_proposal, -1)
        final_features = graph_obj_features.clone()
        final_features[graph_obj_masks] = grpah_features[graph_obj_masks]
        grpah_features = final_features.view(batch_size, num_proposal, self.hidden_size)

        random_numer = random.random()

        grpah_features_output = grpah_features.clone()
        if data_dict["istrain"][0] == 1 and random_numer < 0.5 and not ABLATION.DISABLE_COPY_PASTE:
            obj_masks = objectness_masks.bool().squeeze(2)
            obj_lens = torch.zeros(batch_size, dtype=torch.int).cuda()
            for i in range(batch_size):
                obj_mask = torch.where(obj_masks[i, :] == True)[0]
            ...
            obj_features = obj_features[obj_mask, :].repeat(2,1)
            j = 0
            for i in range(batch_size):
        ...
        data_dict["random"] = random_numer

        feature0 = features.clone()
        if data_dict["istrain"][0] == 1 and data_dict["random"] < 0.5 and not ABLATION.DISABLE_COPY_PASTE:
            obj_masks = objectness_masks.bool().squeeze(2)
            obj_lens = torch.zeros(batch_size, dtype=torch.int).cuda()
            for i in range(batch_size):
                obj_mask = torch.where(obj_masks[i, :] == True)[0]
        ...
        for i in range(self.depth):
            feature1 = self.cross_attn[i](feature1, lang_fea, lang_fea, data_dict["attention_mask"])
            feature1 = self.tgt_cross_attn[i](feature1, tgt_lang_fea, tgt_lang_fea, data_dict["tgt_attention_mask"])
            feature1 = self.adj_cross_attn[i](feature1, adj_lang_fea, adj_lang_fea, data_dict["adj_attention_mask"])

        for i in range(self.depth):
            feature2 = self.cross_attn[i+self.depth](feature2, lang_fea, lang_fea, data_dict["attention_mask"])
            feature2 = self.ngh_cross_attn[i](feature2, ngh_lang_fea, ngh_lang_fea, data_dict["ngh_attention_mask"])
        
        feature_agg = torch.cat((feature1,feature2),dim=-1)
        feature_agg = feature_agg.permute(0, 2, 1).contiguous()

        confidence = self.match(feature_agg).squeeze(1)
