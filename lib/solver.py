...
class Solver():
    ...
    def _backward(self):
        ...
        if self.args.detector == "GF" and self.args.clip_norm > 0:
            grad_total_norm = torch.nn.utils.clip_grad_norm_(self.model.detector.parameters(), self.args.clip_norm)
        self.optimizer.step()
    ...
    def _feed(self, dataloader, phase, epoch_id):
        ...
        if phase == "val":
            ...
            self.my_log['val_loss'].append(self.log[phase]["loss"])
            self.my_log['val_ref_loss'].append(self.log[phase]["ref_loss"])
            ...
            with open(f"{model_root}/epoch_log.json",'w') as f:
                json.dump(self.epoch_log,f)
    ...
    def _epoch_report(self, epoch_id):
        ...
        self.epoch_log['loss_train'].append(round(np.mean([v for v in self.log["train"]["loss"]]), 5))
        self.epoch_log['loss_val'].append(round(np.mean([v for v in self.log["val"]["loss"]]), 5))
        self.epoch_log['loss_ref_train'].append(round(np.mean([v for v in self.log["train"]["ref_loss"]]), 5))
        self.epoch_log['loss_ref_val'].append(round(np.mean([v for v in self.log["val"]["ref_loss"]]), 5))
