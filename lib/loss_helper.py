...
def compute_vote_loss(data_dict):
    """ Vote loss: for a seed point on an object, loss = min_i,j d(v_i, c_j) over its
    predicted votes v_i and the GT votes c_j of any object it may belong to.
    """
    ...
    # gather GT votes at seed indices; mask is 0 for non-object seeds, 1 for object seeds
    ...
    # a vote with no nearby GT match isn't penalized, as long as some vote is near the GT vote
...
def get_loss(args, data_dict, config, detection=True, reference=True, use_lang_classifier=False):
    ...
    if args.detector == "GF":
        ...
        #data_dict['objectness_loss'] = data_dict['sum_heads_objectness_loss']
        ...
        #data_dict['sem_cls_loss'] = data_dict['sum_heads_sem_cls_loss']
        #data_dict['box_loss'] = data_dict['sum_heads_box_loss']
